---
title: Maps
type: index
setting: Aylbyia
tags:
  - setting/aylbyia
  - type/index
  - category/maps
category: Maps
visibility: public
description: Interactive maps for the Aylbyia setting.
related: []
---

# Maps

## Aylbyia world map

Hover a marker for a short location note. Click a marker to open the matching article.

```zoommap
imageBases:
  - path: assets/aylbyia/index/cool-open-world-244aa8afac-full.webp
    name: Aylbyia World Map
markers: settings/Aylbyia/Maps/aylbyia-world.markers.json
minZoom: 0.3
maxZoom: 8
width: 100%
responsive: true
classes:
  - aylbyia-world-map
```

<!-- map-marker-links:start -->
## Mapped locations

### Regions

- [[settings/Aylbyia/Regions/Orkunsteppes|Orkunsteppes]]
- [[settings/Aylbyia/Regions/The Younger Brother|The Younger Brother]]
- [[settings/Aylbyia/Regions/The Platsmoor|The Platsmoor]]
- [[settings/Aylbyia/Regions/The Older Brothers|The Older Brothers]]
- [[settings/Aylbyia/Regions/The Realm of Vuuch|The Realm of Vuuch]]
- [[settings/Aylbyia/Regions/The Skal'ik Desert and Lyzykos|The Skal'ik Desert and Lyzykos]]
- [[settings/Aylbyia/Regions/great houses of humanity/The Great Houses of Humanity|The Great Houses of Humanity]]
- [[settings/Aylbyia/Regions/Fin's End|Fin's End]]
- [[settings/Aylbyia/Regions/Twilight|Twilight]]
- [[settings/Aylbyia/Regions/The Dracopeaks|The Dracopeaks]]
- [[settings/Aylbyia/Regions/Tiulasa|Tiulasa]]
- [[settings/Aylbyia/Regions/The Frosted South|The Frosted South]]
- [[settings/Aylbyia/Regions/The Eastern Continent|The Eastern Continent]]

### Great House subregions

- [[settings/Aylbyia/Regions/great houses of humanity/Northfangs|Northfangs]]
- [[settings/Aylbyia/Regions/great houses of humanity/Sinderealms|Sinderealms]]
- [[settings/Aylbyia/Regions/great houses of humanity/Southfang|Southfang]]

### Settlements

- [[settings/Aylbyia/Settlements/Orkunsteppes/Lor|Lor]]
- [[settings/Aylbyia/Settlements/Orkunsteppes/Unarchus|Unarchus]]
- [[settings/Aylbyia/Settlements/Tavik Nurlowk|Tavik Nurlowk]]
- [[settings/Aylbyia/Settlements/Ziebglen|Ziebglen]]
- [[settings/Aylbyia/Settlements/Junon, The Celestial City|Junon, The Celestial City]]
- [[settings/Aylbyia/Settlements/Fin's end/Castle Bleakunpine|Castle Bleakunpine]]
- [[settings/Aylbyia/Settlements/Fin's end/Castle Sobet|Castle Sobet]]
- [[settings/Aylbyia/Settlements/Fin's end/Tritonya|Tritonya]]
- [[settings/Aylbyia/Settlements/Drakopeaks/South Harbour|South Harbour]]
- [[settings/Aylbyia/Settlements/The great houses of Humanity/The House of Water|The House of Water]]
- [[settings/Aylbyia/Settlements/The great houses of Humanity/Southfang/Port Shockdark|Port Shockdark]]
- [[settings/Aylbyia/Settlements/The great houses of Humanity/Southfang/Tavik Southfang|Tavik Southfang]]

### Map features

- [[settings/Aylbyia/Regions/Map Features/Advenza|Advenza]]
- [[settings/Aylbyia/Regions/Map Features/Oarm Sea|Oarm Sea]]
- [[settings/Aylbyia/Regions/Map Features/The Blazing Straits|The Blazing Straits]]
- [[settings/Aylbyia/Regions/Map Features/Goblin Bay|Goblin Bay]]
- [[settings/Aylbyia/Regions/Map Features/Cobalt Bay|Cobalt Bay]]
- [[settings/Aylbyia/Regions/Map Features/Haerone Forest|Haerone Forest]]
- [[settings/Aylbyia/Regions/Map Features/Pale Gate|Pale Gate]]
- [[settings/Aylbyia/Regions/Map Features/Black Mountain|Black Mountain]]
- [[settings/Aylbyia/Regions/Map Features/Lake Koral|Lake Koral]]
- [[settings/Aylbyia/Regions/Map Features/Bay of Peril|Bay of Peril]]
- [[settings/Aylbyia/Regions/Map Features/The Smoulderspine|The Smoulderspine]]
- [[settings/Aylbyia/Regions/Map Features/The Silent Promise|The Silent Promise]]
- [[settings/Aylbyia/Regions/Map Features/The Shell|The Shell]]
- [[settings/Aylbyia/Regions/Map Features/Court of Mer|Court of Mer]]
- [[settings/Aylbyia/Regions/Map Features/Mouth of Creation|Mouth of Creation]]
- [[settings/Aylbyia/Regions/Map Features/The Unholy Sepulcher|The Unholy Sepulcher]]
- [[settings/Aylbyia/Regions/Map Features/The Hanging Tree|The Hanging Tree]]
- [[settings/Aylbyia/Regions/Map Features/The Roii|The Roii]]
<!-- map-marker-links:end -->

## Map label audit

These visible labels are not mapped yet because they are either ambiguous, too local for the world-map layer, or need a more confident article/position before becoming clickable:

- Smaller town labels around Platsmoor, Lake Koral, and the Great Houses — need article decisions.
- Frosted-south settlements other than [[settings/Aylbyia/Settlements/The Frosted South/The Citadel of Frozen Crystal|the Citadel of Frozen Crystal]] — need position confirmation.
- Labels in the western corrupted/desert region — need name confirmation before public articles.

<!-- curated-index:start -->
<section class="index-guide">
  <p class="index-guide-kicker">Maps</p>
  <p class="index-guide-purpose">Interactive maps for the Aylbyia setting.</p>
  <div class="index-route-grid index-route-grid-compact">
    <a class="index-route-card" href="#mapped-locations"><span>Mapped links</span><strong>Mapped locations</strong><em>Jump to linked regions, settlements, and map features.</em></a>
    <a class="index-route-card" href="#map-label-audit"><span>Audit</span><strong>Map label audit</strong><em>See labels that still need article or position decisions.</em></a>
  </div>
</section>
<!-- curated-index:end -->

<!-- vault-folder-index:start -->

<!-- vault-folder-index:end -->
